...
\begin{observation}
Individual \ac{RCE} instances are spawned as subprocesses of the main \ac{RCE} instance by the \ac{IM} to emulate a distributed system locally. Their communication takes place over the loopback device.
\end{observation}
...
\begin{observation}
    The manual termination of instances during test execution not only caused test failures but also went unnoticed by the \ac{IM}.
\end{observation}
...
\begin{observation}
    Although the implementation of the \ac{IM} is decoupled through an interface, this interface exhibits a bias towards the implementation for local execution of RCE instances.
\end{observation}
